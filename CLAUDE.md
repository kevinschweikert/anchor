...
**Browser-safety invariant for `shared/` (and all JS-target packages):** ids are `String`, not `uuid.Uuid` — youid pulls in `gleam_crypto`, whose JS build targets Node crypto APIs (`import "node:crypto"`). Per gleam_crypto's own docs these exist in server runtimes (Node, Bun, Deno) but **not** in web browsers. This is a **browser-runtime** failure, not a build or test failure: the package compiles to JS fine and runs under Node, so `gleam test` (which runs on Node) passes with it in the graph — but the shipped widget/app bundle crashes when loaded in a browser. ES module imports are eager, so merely _importing_ a Node-only module anywhere in the import graph breaks the bundle at load time, even if the function is never called. Keep `youid` (and other Node/Erlang-only deps) out of `shared/` and the JS packages; UUIDs are generated server-side and cross the wire as strings.
